...
var weekString = "Sem";
var scrollLeftMessage = "Click para ir al mes anterior. Poner el mouse para el scroll automático.";
var scrollRightMessage = "Click para ir al mes siguiente. Poner el mouse para el scroll automático.";
...
var crossobj=null, crossMonthObj, crossYearObj, monthSelected, yearSelected, dateSelected, omonthSelected, oyearSelected, odateSelected, monthConstructed, yearConstructed, intervalID1, intervalID2, timeoutID1, timeoutID2, ctlToPlaceValue, ctlNow, dateFormat, nStartingYear;
var k,i,j;
...
if (dom){
    ...
    document.write ("<div onclick='bShow=true' id='calendar' style='z-index:+999;position:absolute;visibility:hidden;'><table width="+((showWeekNumber==1)?250:220)+
        " style='font-family:arial;font-size:11px;border-width:1;border-style:solid;border-color:#a0a0a0;font-family:arial; font-size:11px}' bgcolor='#ffffff'><tr background='"+imgDir+"calback.jpg'><td background='"+imgDir+"calback.jpg'><table width='"+((showWeekNumber==1)?248:218)+
        "'><tr><td style='padding:2px;font-family:arial; font-size:11px;'><font color='#FFFFFF'><B><span id='caption'></span></B></font></td><td align=right><a href='javascript:hideCalendar()'><IMG SRC='"+imgDir+
        "close.gif' BORDER='0' ALT='Cerrar al Calendario'></a></td></tr></table></td></tr><tr><td style='padding:5px' bgcolor=#ffffff><span id='content'></span></td></tr>")
    if (showToday==1){
...
function hideCalendar() {
    if(crossobj=='undefined' || crossobj==null)
        return;
...
function popUpMonth() {
    ...
    var g=parseInt(crossobj.left) + 50;
    crossMonthObj.left = g+"px";
    //    crossMonthObj.left = crossobj.left;
    var h=parseInt(crossobj.top) + 26;
    crossMonthObj.top = h+"px"
    //    crossMonthObj.left = crossobj.top;
...
function popUpYear() {
    ...
    //    leftOffset = crossobj.left;
    if (ie){
        leftOffset += 6
    }
    crossYearObj.left = leftOffset+"px"
    var s=parseInt(crossobj.top) +26;
    crossYearObj.top = s+"px"
//    crossYearObj.top = crossobj.top;
}
/*** calendar ***/
function WeekNbr(n) {
    // Algorithm used:
...
function popUpCalendar(ctl, ctl2, format) {
    ...
    if (bPageLoaded){
        if ( crossobj.visibility == "hidden" ) {
            ...
            var left = fixedX==-1 ? ctl.offsetLeft + leftpos : fixedX;
            var right = fixedY==-1 ? ctl.offsetTop + toppos + ctl.offsetHeight + 2 : fixedY;
...
function padNmb(nStr, nLen, sChr){
    ...
    for (i = 0; i < nLen - String(nStr).length; i++)
        sRes = sChr + sRes;
    return sRes;
}
...
function addToDate(sFec0, sInc){
    ...
    var sRes = sFec0;
    if (parseInt(sInc) >= 0)
        for ( i = 0; i < nInc; i++) sRes = incDate(sRes);
    else
        for (i = 0; i < nInc; i++) sRes = decDate(sRes);
    return sRes;
}
function recalcF1(){
