        ...
        try {
            ...
            if (op.equals("1")) {
                ...
                if (tipocuenta.equals("CUENTA A PLAZO FIJO")) {
                    ...
                    dao.update();
                    BigDecimal mx = cp.getSaldo().subtract(cp.getSaldoSinInteres());
                    if (!mx.equals(BigDecimal.ZERO)) {
                        boolean tryied = CheckCapital.capitalizarForced(session, new Date(), idcuenta, mx.doubleValue());
                    }
                    //DateUtil.eedatefmt(DateUtil.fmtdate24, Double.parseDouble(detalleCuentPers.getFechaPlazo())).replace(",", "")
                } else {
                ...
                TCuentaPersona cp = (TCuentaPersona) dao.load(TCuentaPersona.class, idcuenta);
                if (cp.getEstado().equals("BLOQUEADO")) {
                    if (cp.getSaldo().doubleValue() >= 0) {
                        cp.setEstado("ACTIVO");
                    } else {
                        cp.setEstado("SOBREGIRO");
                    }
                } else {
                    cp.setEstado("BLOQUEADO");
                }
                dao.update();
            }
            out.print("OK");
